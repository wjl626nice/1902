...
layui.extend({
  ...
  ,entryPage = function(fn){
    ...
    if(isIndPage || pathURL === '/user/login'){ //此处单独判断登入页，是为了兼容旧版（即未在 config.js 配置 indPage 的情况）
      ...
      if(setter.interceptor){
        ...
        var local = layui.data(setter.tableName); //从localStorage中获取存在的所有数据
        // setter.request.tokenName  = 'access_token'
        // 检查令牌是否在localStorage值中存在
  ...
  //对外输出
  //给当前的模块起个名称叫 index  第二个参数暴露出去让外部调用
  exports('index', {
    render: renderPage,
  });
});
